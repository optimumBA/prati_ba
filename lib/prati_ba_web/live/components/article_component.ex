defmodule PratiBaWeb.Components.ArticleComponent do
  ...
  defp article_details(assigns) do
    ...
    <div class="w-3/4 text-left relative flex flex-col justify-between">
      <div class="text-sm sm:text-[20px] lg:text-lg font-semibold lg:font-medium overflow-hidden leading-tight pl-2 lg:min-h-[145px] dark:text-gray-300">
        <h1><%= @article.title %></h1>
      </div>
      ...
      <div class="flex items-center details pt-1 sm:pt-2 text-gray-10 dark:text-gray-20 text-xs sm:text-sm lg:text-base pl-2">
        <span class="flex items-center">
          <Icons.article_link />
          <span class="article-source border-b text-center border-b-gray-10">
            <%= @article.source.name %>
          </span>
        </span>
  ...
  defp relative_publish_time(%{article: %Article{published_at: published_at}} = assigns) do
    ...
    ~H"""
    <time class="block sm:hidden article-published truncate overflow-hidden" datetime={@datetime}>
      <%= short_relative_publish_time(@relative_string) %>
    </time>
    <time class="hidden sm:block article-published truncate overflow-hidden" datetime={@datetime}>
      <%= @relative_string %>
    </time>
    """
  end
  ...
  def navbar(assigns) do
    ...
    <div id="nav-ignore" class="w-full" phx-update="ignore">
      <div
        id="nav-hook"
        class="w-full fixed top-0 flex items-center justify-center bg-white dark:bg-[#212936] z-50 border-b border-gray-10 dark:border-gray-30"
        phx-hook="HeaderHook"
      >
        <nav id="navbar" role="navigation" class="w-full flex items-center md:w-10/12 xl:w-[907px]">
          <section class="w-full">
            <div
              id="theme-toggle"
              class="ml-6 cursor-pointer w-max text-gray-600 dark:text-gray-20"
              phx-hook="ToggleBgHook"
            >
          ...
          <section class="w-max pt-2">
            <.link href={~p"/"} class="block w-max mx-auto">
              <img
                ...
                width="149"
                height="64"
                class="w-24 sm:w-36 dark:hidden block sm:py-4 py-2 duration-200"
              />
              <img
                id="app-logo-dark"
                ...
                width="149"
                height="64"
                class="w-24 sm:w-36 dark:block hidden sm:py-4 py-2 duration-200"
              />
            </.link>
          </section>
